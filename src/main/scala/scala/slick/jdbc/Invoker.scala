...
trait Invoker[+R] { self =>
  ...
  def mapResult[U](f: (R => U)): Invoker[U] = new MappedInvoker(this, f)
...
class MappedInvoker[U, +R](parent: Invoker[U], mapper: (U => R)) extends Invoker[R] {
  def iteratorTo(maxRows: Int)(implicit session: JdbcBackend#Session) =
    parent.iteratorTo(maxRows).map(mapper)
}
